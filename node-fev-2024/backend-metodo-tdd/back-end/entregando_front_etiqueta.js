...
export function entregando_front_etiqueta(req, res, next) {

       
       var dir = path.join(__dirname, local_index); 
       
       var previo_file = path.join(dir, "index.html");   

//       path.join(dir, req.path.replace(/\/$/, '/index.html'));   

//console.log("  previo_file "+dir);
...
//console.log(req.path+"     entregando_front_etiqueta  *************************    "+previo_file)
   ...
   //Em caso de ERRO deve-se liberar para o seguinte:
       stream_pastas_locais.on('error', function (err) {
           res.send("Erro função (entregando_front_etiqueta) "+req.path);
           next();
       });
   }
